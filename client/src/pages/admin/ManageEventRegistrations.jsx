...
const ManageEventRegistrations = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await axios.get(getApiUrl('/admin/event-registrations'), getAuthHeaders());
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(getApiUrl(`/admin/event-registrations/${id}`), getAuthHeaders());
            fetchData();
        } catch (error) {
                        ...
                        <table className="admin-table">
                            ...
                            <tbody>
                                {registrations.map(item => (
                                    <tr key={item.id}>
                                        <td data-label="Ticket #" style={{ fontWeight: 600, color: '#fbbf24' }}>#{item.ticket_number}</td>
                                        <td data-label="Name" style={{ fontWeight: 500, color: '#fafafa' }}>{item.name}</td>
