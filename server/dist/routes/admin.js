...
router.post('/login', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: user.id.toString(), email: user.email }, JWT_SECRET, { expiresIn: '24h' });
...
router.post('/popups', upload_1.upload.fields([{ name: 'image', maxCount: 1 }]), async (req, res) => {
    try {
...
router.post('/life-at-xaviers', upload_1.upload.fields([
    ...
    try {
        ...
        const files = req.files;
        const saveMedia = async (file, collectionName) => {
            const mediaRecord = await prisma.media.create({
                data: {
...
router.delete('/event-registrations/:id', async (req, res) => {
    try {
        await prisma.event_registrations.delete({ where: { id: BigInt(req.params.id) } });
        res.json({ success: true });
    }
    catch (e) {
        res.status(500).json({ error: 'Server error' });
...
router.delete('/metas/:id', async (req, res) => {
    try {
        await prisma.metas.update({ where: { id: BigInt(req.params.id) }, data: { deleted_at: new Date() } });
        res.json({ success: true });
    }
    catch (e) {
        res.status(500).json({ error: 'Server error' });
